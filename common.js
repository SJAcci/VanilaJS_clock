...
function clock(){ //생각만 하고있는 상태 => 여러번 호출
    //현재 날짜와 시간 담기
    var now = new Date(); 
    ...
    var day = now.getDay(); //0-6사이의 숫자로 반환됨
    ...
    for(var i in dateObject){ //3번 작성하지 않고 한번에 텍스트컨텐츠로 추가
...
//1초마다 변경
setInterval(clock,1000); //콜백함수 자리에 함수를 불러올 때 ()를 작성하지 않음
